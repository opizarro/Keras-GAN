...
#        y_train = y_train.reshape(-1, 1)
        ...
        for epoch in range(epochs):
            ...
            d_loss_real = self.discriminator.train_on_batch([imgs, bpatches, bp_means], valid)
            d_loss_fake = self.discriminator.train_on_batch([gen_imgs, bpatches, bp_means], fake)
            ...
            if epoch % sample_interval == 0:
                ...
                plt.show()
                plt.savefig("/data/metrics_cgan/metrics.png")
                plt.close()
    ...
    def sample_images(self, epoch, Xbathy_samples, Xbathy_samples_means):
        ...
        for i in range(r):
            for j in range(c):
                ...
                cnt += 1
        fig.savefig("/data/images_cgan/%d.png" % epoch)
        plt.close()
