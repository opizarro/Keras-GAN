...
class AdversarialAutoencoder():
    def __init__(self):
       ...
       # try:
        ...
        self.discriminator_cat.compile(loss='categorical_crossentropy',
            optimizer=optimizer,
            metrics=['accuracy'])

        # Build the encoder / decoder
        self.encoder = self.model_encoder_cat()
        self.decoder = self.model_generator_cat()
        #self.decoder = self.build_decoder2()

        img = Input(shape=self.img_shape)
        # The generator takes the image, encodes it and reconstructs it
        # from the encoding
        encoded_repr, encoded_cat = self.encoder(img)
        reconstructed_img = self.decoder(Concatenate([encoded_repr, encoded_cat]))
        ...
        # Cat validity
        validity_cat = self.descriminator_cat(encoded_cat)
        ...
        self.adversarial_autoencoder.compile(loss=['mse', 'binary_crossentropy', 'categorical_crossentropy'],
            loss_weights=[1e3, 1e-1, 1e-1],
            optimizer=optimizer)
    ...
    def model_encoder_cat(self, units=512, reg=lambda: regularizers.l1_l2(l1=1e-7, l2=1e-7), dropout=0.5):
        k = 5
        x = Input(shape=self.img_shape)
        h = Conv2D(units// 4, (k, k), padding='same', kernel_regularizer=reg())(x)
        # h = SpatialDropout2D(dropout)(h)
        ...
        z = Lambda(lambda ml: ml[0] + K.random_normal(K.shape(ml[0])) * K.exp(ml[1] / 2),
                   output_shape=lambda ml: ml[0])([mu, log_sigma_sq])
        cat = Dense(self.num_cats,activation='softmax')(h)

        return Model(x, outputs=[z, cat], name="encoder")
    ...
    def model_generator_cat(self, units=512, dropout=0.5, reg=lambda: regularizers.l1_l2(l1=1e-7, l2=1e-7)):

        h = 5

        decoder.add(Dense(units * 4 * 4 , input_dim=self.latent_dim+self.num_cats, kernel_regularizer=reg()))
        # check channel order on below
        decoder.add(Reshape((4,4,units)))
        # decoder.add(SpatialDropout2D(dropout))
        #decoder.add(LeakyReLU(0.2))
        ...
        z = Input(shape=(self.latent_dim+self.num_cats,))
        img = decoder(z)
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            latent_real_cat = np.random.multinomial(size=(desc_batch, self.num_cats))
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("images_generator_cat/benthic_%d.png" % epoch)
        plt.close()
    ...
    def sample_autoencoder(self, epoch,imgs):
        ...
        for i in range(r):
            for j in range(c):
                ...
                cnt += 1
        fig.savefig("images_autoenc_cat/benthic_%d.png" % epoch)
        plt.close()
